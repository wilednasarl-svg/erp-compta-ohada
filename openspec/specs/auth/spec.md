...
## Purpose
Authentification email + mot de passe (argon2id), émission et rotation des tokens JWT (access HS256, refresh opaque) avec détection de réutilisation, MFA TOTP (RFC 6238) avec codes de secours, et journal d'événements d'authentification (`auth_events`) en append-only. Toutes les autres capabilities (RBAC, organisations, modules métier) dépendent de cette base.
## Requirements
